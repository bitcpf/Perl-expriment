...

{
	...
	elsif ($filename =~ /^iperf_tcp_7/)
	{
		push(@filearray_70,"$filename");
	}
	elsif ($filename =~ /^iperf_tcp_9/)
	{
...
#11:18:35.649386 5216232us tsft 6.0 Mb/s antenna 1 [bit 15] 60us DA:00:15:6d:69:de:89 (oui Unknown) SA:06:15:6d:68:4e:3f (oui Unknown) BSSID:00:00:00:00:00:00 (oui Ethernet) LLC, dsap SNAP (0xaa) Individual, ssap SNAP (0xaa) Command, ctrl 0x03: oui Ethernet (0x000000), ethertype Unknown (0x0008): Unnumbered, ui, Flags [Command], length 36
			if ($line =~ /\s*(\d+):(\d+):(\d+).(\d+)\s(\d+)us\stsft\s+(\d+.\d+)\sMb\/s\santenna\s\d\s\[bit 15\].*/)#
#         10  11                              12                                     
			{
...
#00:09:47.213304 590680576us tsft short preamble 1.0 Mb/s 2412 MHz 11g -87dB signal -96dB noise antenna 1 9dB signal 120us RA:e0:46:9a:3c:f9:bd (oui Unknown) Clear-To-Send
	...
	elsif ($line =~ /.*signal\s(\d+)us.*Clear-To-Send\S*/)
...
#		print OUTFILELOG2 $1;
		...
		$activity=$activity+(61*8/1/1000000)+$1/1000000;
}
		$inter_c2=$inter_c2+1;
...
if ($line =~ /\.*GPGGA,(\d+).(\d+),(\d+).(\d+),N,(\d+).(\d+),W,(\d+),(\d+),(\d+).(\d+),(\d+),M.*/)
{
	...
	our $longitude = $5.".".$6;
		...
		$latitude_cent=($latitude-$latitude_degree*100)/60;
		$latitude=$latitude_degree+$latitude_cent;
		
		$longitude_degree=int($longitude/100);
		$longitude_cent=($longitude-$longitude_degree*100)/60;
		$longitude=0-($longitude_degree+$longitude_cent);
...
#print OUTFILELOG $inter_cnt,",",$activity,"\n";
	...
	if($time eq "103453" || $time eq "103455")
	{
		print OUTFILELOG2 "$time,$inter_c1,$inter_c2,$inter_c3,$inter_c4,$inter_c5,$inter_c6,$inter_c7\n";
