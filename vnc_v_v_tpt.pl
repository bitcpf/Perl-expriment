...
# Modified 20/09/2012
# Modified 13/10/2012
# Purpose:  Parse tcpdump data for multiband antenna experiments:Simultaneous non-802.11 
# Usage: tests 
use Switch;
...
###########################################################################################
my $iperf_24="throughput_24";
my $iperf_52="throughput_52";
my $iperf_70="throughput_70";
my $iperf_90="throughput_90";
...
{
	if ($filename =~ /^iperf_tpt_2/)
	{
		push(@filearray_24,"$filename");
	}
	elsif ($filename =~ /^iperf_tpt_5/)
	{
		push(@filearray_52,"$filename");
	}
	elsif ($filename =~ /^iperf_tpt_7/)
	{
		push(@filearray_70,"$filename");
	}
	elsif ($filename =~ /^iperf_tpt_9/)
	{
...
if($throughput eq "0.00") {$throughput = 0.001;
...
#Output here	
				...
				print OUTFILEIPERF $gpstime,",",$velocity,",",$latitude,",",$longitude,",",$throughput,"\n";
